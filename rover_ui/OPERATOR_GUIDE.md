...
| **tag geometry** | How well-conditioned the tags currently in view are, which is what decides whether a fix is possible at all. `baseline 0.24 (250mm @ 1050mm)` is the angular baseline — the horizontal spread of the tags divided by their range. Above `TAGS_MIN_SPREAD_RATIO` (0.18) the geometry is usable however narrow the spread looks; well below it, no fix can be trusted from that view whatever the reprojection error says. Also shows `window n/m agree` (how many recent observations corroborate each other), `holding: …` when a correction is waiting for corroboration, and the last micro-parallax outcome. |
...
### 5.2a ⇄ Micro-parallax fix

Calls `POST /api/nav/parallax`. **This button moves the rover**: it strafes
roughly 120–350 mm sideways, takes a second look at the same tags, and (by
default) strafes back.

Use it when the tag-geometry readout says the view is degenerate — typically
when the rover is close to a panel and only one vertical column of tags is in
frame. That view carries no information about sideways position at all: the
solver can trade a sideways shift against a heading change and reproject just
as well either way, so it returns a confident, wrong answer. No threshold can
recover what was never measured, which is why such frames are refused rather
than corrected.

The fix is to create the missing angle rather than wait for it. Two views of
the same tags taken from positions a known distance apart carry the same
information a wider tag layout would, and the T265 measures that short
displacement accurately (it drifts over minutes, not over 200 mm). The pair is
then fused into one solve. Measured on a single two-tag column, median lateral
error, one view versus the fused pair: 24 mm → 8 mm at 1.6 m, 100 mm → 26 mm at
2.5 m.

The jog is sized from the estimated range (a fixed distance would be plenty
close up and useless far away), and the direction is whichever side has clear
floor. It is refused while a navigation is running, and the manoeuvre is
abandoned — with the rover returned to where it started — if the heading moves
during the jog, if the rover does not actually move, or if the second view
loses the tags.

**It also runs by itself.** With `TAGS_PARALLAX_AUTO` enabled (the default),
the rover attempts this automatically after `TAGS_PARALLAX_TRIGGER_N` (3)
consecutive degenerate-geometry frames while it is stopped, at most once every
`TAGS_PARALLAX_COOLDOWN_S` (20 s). So a parked rover in front of a single tag
column will occasionally shuffle sideways and back on its own. That is the
system rescuing a fix it otherwise could not take — but if unprompted movement
is unacceptable during a demo, set `TAGS_PARALLAX_AUTO=0` and drive it from
this button instead.
...
| `POST /api/nav/parallax` | `{baseline_mm?, direction?}` | Take a micro-parallax fix: strafe a known distance, fuse the two views. Moves the rover; refused mid-nav |
...
| `tag_live` | The current solve whether or not it was applied, including `spread_mm`, `range_mm` and `spread_ratio` — the angular baseline the gate reasons about |
| `tag_window` | `{n, agree, span_s, strict, loose}` — the rolling window of recent observations backing (or failing to back) a correction |
| `tag_pending` | `{need, tier, why}` when a correction is being held for corroboration, else `null`. This is the answer to "why hasn't it corrected yet" |
| `tag_fix` | Last **applied** fix. Now also carries `source` (`tag` or `parallax`), `tier` (`strict`/`loose`) and `consensus` (`{n, need, span_s, scatter_mm, dropped}`) — how many observations agreed, over what span, and how tightly |
| `tag_parallax` | Last micro-parallax attempt: `{ok, baseline_mm, range_mm, yaw_drift_deg, returned, ids_a, ids_b, why?}` |
| `parallax_busy` | True while a parallax manoeuvre owns the rover |
| `degenerate_run` | Consecutive frames refused for geometry; at `TAGS_PARALLAX_TRIGGER_N` the auto-parallax fires |
...
|---|---|
| Tags visible but **no fixes at all**, tag geometry shows a low `baseline` | The view is degenerate: one vertical column of tags carries no sideways information, so it is refused rather than trusted. Move so both columns of a panel are in shot, get closer (the same spread is worth more at short range), or press **⇄ Micro-parallax fix** (§5.2a) to make the missing baseline. |
| A correction is clearly needed but never applies; `holding: n of m agreeing` | The fix is waiting for corroboration, not failing. Loose-tier frames need `TAGS_LOOSE_CONSENSUS_N` agreeing observations, and any correction over `TAGS_BIG_FIX_MM` needs `TAGS_CONFIRM_N`. If the window keeps showing disagreement, the tags themselves are unstable — check for glare or a moved panel. |
| The rover shuffles sideways on its own while parked | Auto micro-parallax (§5.2a). It fires after 3 consecutive degenerate-geometry frames while stopped. Set `TAGS_PARALLAX_AUTO=0` to make it button-only. |
